...
/// Renders an Uthmani ayah with character-level Tajweed coloring, scaling with Dynamic
/// Type. Color is never the only signal: a VoiceOver summary names the flagged letters,
/// and the parent screen shows a symbol legend for color-blind users.
...
/// CoreText-backed label. A single attributed string is shaped and drawn as one run, so
/// Arabic ligatures and RTL ordering survive the per-character coloring. Height is
/// reported via `CTFramesetterSuggestFrameSizeWithConstraints` so SwiftUI lays it out
/// and it grows with Dynamic Type.
...
final class CoreTextLabelView: UIView {
    ...
    override func traitCollectionDidChange(_ previous: UITraitCollection?) {
        ...
        // Re-resolve dynamic (light/dark) colors.
        setNeedsDisplay()
    }
